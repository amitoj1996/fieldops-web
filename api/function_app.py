...
def list_tasks(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if _is_admin(pr):
            ...
            me = (pr.get("userDetails") or pr.get("userId") or "").lower()
            q = ("SELECT * FROM c WHERE c.tenantId=@t AND c.docType='Task' "
                 "AND LOWER(c.assignee)=@me ORDER BY c.createdAt DESC")
            items = list(c.query_items(q, parameters=[{"name":"@t","value": tenant},{"name":"@me","value": me}],
...
def tasks_update(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(json.dumps({"error": str(e)}),
                                 mimetype="application/json", status_code=500)
# ======== USERS: invite & membership (fits your helpers) ========
import hmac, hashlib, base64, json, time

def _users_container():
    return _get_container_named(os.environ.get("USERS_CONTAINER", "Users"))

def _email_from_pr(pr):
    return (pr.get("userDetails") or pr.get("userId") or "").strip().lower()
...
def _sign_invite(payload: dict):
    secret = os.environ.get("INVITE_SECRET")
    if not secret:
        raise RuntimeError("Missing INVITE_SECRET")
    data = json.dumps(payload, separators=(",", ":")).encode("utf-8")
    sig  = hmac.new(secret.encode("utf-8"), data, hashlib.sha256).digest()
    return _b64url(data) + "." + _b64url(sig)

def _verify_invite(token: str):
    secret = os.environ.get("INVITE_SECRET")
    if not secret:
        return None
    try:
        db64, sb64 = token.split(".", 1)
        data = _ub64url(db64); sig = _ub64url(sb64)
        exp_sig = hmac.new(secret.encode("utf-8"), data, hashlib.sha256).digest()
        if not hmac.compare_digest(sig, exp_sig):
            return None
        obj = json.loads(data.decode("utf-8"))
        if obj.get("exp") and time.time() > float(obj["exp"]):
            return None
        return obj
    except Exception:
        return None

def _is_member(email: str, tenant: str = "default"):
    c = _users_container()
    q = ("SELECT TOP 1 * FROM c WHERE c.docType='User' AND c.tenantId=@t "
         "AND c.email=@e AND c.status='ACTIVE'")
    it = list(c.query_items(q, parameters=[{"name":"@t","value":tenant},{"name":"@e","value":email}],
                            enable_cross_partition_query=True))
    return it[0] if it else None

@app.route(route="users", methods=["GET"])
def users_list(req: func.HttpRequest) -> func.HttpResponse:
    _, err = _ensure_admin(req)
    if err: return err
    try:
        tenant = req.params.get("tenantId","default")
        c = _users_container()
        q = "SELECT * FROM c WHERE c.docType='User' AND c.tenantId=@t ORDER BY c.email"
        items = list(c.query_items(q, parameters=[{"name":"@t","value":tenant}],
                                   enable_cross_partition_query=True))
        return func.HttpResponse(json.dumps(items), mimetype="application/json", status_code=200)
    except Exception as e:
        return func.HttpResponse(json.dumps({"error": str(e)}), mimetype="application/json", status_code=500)
...
def users_upsert(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        status = (data.get("status") or "INVITED").upper()
        if not email:
            return func.HttpResponse(json.dumps({"error":"email required"}), mimetype="application/json", status_code=400)
        c = _users_container()
        q = "SELECT TOP 1 * FROM c WHERE c.docType='User' AND c.tenantId=@t AND c.email=@e"
        it = list(c.query_items(q, parameters=[{"name":"@t","value":tenant},{"name":"@e","value":email}],
                                enable_cross_partition_query=True))
        if it:
            u = it[0]
            u["roles"] = roles
            u["status"] = status
            u["updatedAt"] = _now_iso()
            c.replace_item(item=u, body=u)
            return func.HttpResponse(json.dumps(u), mimetype="application/json", status_code=200)
        else:
            u = {
                "id": str(uuid.uuid4()),
                "docType": "User",
            ...
            c.create_item(u)
            return func.HttpResponse(json.dumps(u), mimetype="application/json", status_code=201)
    except Exception as e:
        return func.HttpResponse(json.dumps({"error": str(e)}), mimetype="application/json", status_code=500)

@app.route(route="users/invite", methods=["POST"])
def users_invite(req: func.HttpRequest) -> func.HttpResponse:
    _, err = _ensure_admin(req)
    if err: return err
    try:
        data = req.get_json()
        ...
        if it:
            ...
            c.create_item(u)
        payload = {"tenantId": tenant, "email": email, "roles": roles, "exp": time.time() + 7*24*3600}
        token = _sign_invite(payload)
        invite_path = f"/employee?invite={token}"
        return func.HttpResponse(json.dumps({"invitePath": invite_path, "email": email}),
                                 mimetype="application/json", status_code=200)
    except Exception as e:
        return func.HttpResponse(json.dumps({"error": str(e)}), mimetype="application/json", status_code=500)

@app.route(route="users/accept", methods=["GET","POST"])
def users_accept(req: func.HttpRequest) -> func.HttpResponse:
    pr, err = _ensure_auth(req)
    if err: return err
    try:
        if req.method == "GET":
            tenant = req.params.get("tenantId","default")
            token  = req.params.get("token") or req.params.get("invite")
        else:
            body   = req.get_json() or {}
            tenant = body.get("tenantId","default")
            token  = body.get("token") or body.get("invite")
        if not token:
            return func.HttpResponse(json.dumps({"error":"token required"}), mimetype="application/json", status_code=400)
        obj = _verify_invite(token)
        if not obj:
            return func.HttpResponse(json.dumps({"error":"invalid or expired token"}), mimetype="application/json", status_code=400)
        if obj.get("tenantId") != tenant:
            return func.HttpResponse(json.dumps({"error":"tenant mismatch"}), mimetype="application/json", status_code=400)
        caller = _email_from_pr(pr)
        if caller != (obj.get("email") or "").lower():
            return func.HttpResponse(json.dumps({"error":"this invite is for a different account/email"}),
                                     mimetype="application/json", status_code=403)
        c = _users_container()
        q = "SELECT TOP 1 * FROM c WHERE c.docType='User' AND c.tenantId=@t AND c.email=@e"
        it = list(c.query_items(q, parameters=[{"name":"@t","value":tenant},{"name":"@e","value":caller}],
                                enable_cross_partition_query=True))
        if not it:
            return func.HttpResponse(json.dumps({"error":"user record missing"}), mimetype="application/json", status_code=404)
        u = it[0]
        u["status"] = "ACTIVE"
        u["roles"]  = obj.get("roles") or u.get("roles") or ["employee"]
        u["activatedAt"] = _now_iso()
        c.replace_item(item=u, body=u)
        return func.HttpResponse(json.dumps({"ok": True, "member": u}),
                                 mimetype="application/json", status_code=200)
    except Exception as e:
        return func.HttpResponse(json.dumps({"error": str(e)}), mimetype="application/json", status_code=500)

@app.route(route="users/me", methods=["GET"])
def users_me(req: func.HttpRequest) -> func.HttpResponse:
    pr, err = _ensure_auth(req)
    if err: return err
    try:
        tenant = req.params.get("tenantId","default")
        email  = _email_from_pr(pr)
        doc    = _is_member(email, tenant)
        return func.HttpResponse(json.dumps({"email":email,"isAdmin":_is_admin(pr),"isMember":bool(doc),"member":doc}),
                                 mimetype="application/json", status_code=200)
    except Exception as e:
        return func.HttpResponse(json.dumps({"error": str(e)}), mimetype="application/json", status_code=500)
# ======== end USERS block ========
